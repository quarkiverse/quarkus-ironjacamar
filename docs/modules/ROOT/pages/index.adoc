...
IronJacamar is a Java Connector Architecture (JCA) implementation.
...
@ResourceEndpoint(activationSpec = @ActivationSpec(configKey = "myqueue")) <1>
public class MyResourceEndpoint implements MessageListener {...} <2>

----

<1> An optional activation spec configuration key. The `configKey` is the key that will be consumed from the configuration (see the `application.properties` example below)
