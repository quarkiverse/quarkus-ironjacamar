...
public class IronJacamarRecorder {
    ...
    public void initDefaultBootstrapContext(BeanContainer beanContainer) {
        ...
        ExecutorService defaultExecutor = beanContainer.beanInstance(ManagedExecutor.class);
        workManager.setShortRunningThreadPool(defaultExecutor);
        workManager.setLongRunningThreadPool(defaultExecutor);

        // Register the default bootstrap context
        BaseCloneableBootstrapContext defaultBootstrapContext = new BaseCloneableBootstrapContext();
        defaultBootstrapContext.setName(DEFAULT_BOOTSTRAP_CONTEXT_NAME);
        defaultBootstrapContext.setWorkManager(workManager);
        defaultBootstrapContext.setWorkManagerName(DEFAULT_WORK_MANAGER_NAME);
        defaultBootstrapContext
                .setTransactionSynchronizationRegistry(transactionIntegration.getTransactionSynchronizationRegistry());
        defaultBootstrapContext.setXATerminator(transactionIntegration.getXATerminator());

        // Register the default in the coordinator
        WorkManagerCoordinator.getInstance().setDefaultWorkManager(workManager);
        BootstrapContextCoordinator.getInstance().setDefaultBootstrapContext(defaultBootstrapContext);
    }

    /**
     * Initialize the virtual thread {@link CloneableBootstrapContext} and {@link WorkManagerImpl}
     *
     * @param beanContainer The bean container
     */
    public void initVirtualThreadWorkManager(BeanContainer beanContainer) {
        SecurityIntegration securityIntegration = beanContainer.beanInstance(QuarkusSecurityIntegration.class);

        // Create WorkManagerImpl
        WorkManagerImpl workManager = new WorkManagerImpl();
        workManager.setName(Defaults.VIRTUAL_THREAD_WORK_MANAGER_NAME);
        workManager.setSpecCompliant(true);
        workManager.setSecurityIntegration(securityIntegration);
        ...
        // Register the default in the coordinator
        WorkManagerCoordinator.getInstance().registerWorkManager(workManager);
    }
    ...
    public RuntimeValue<Future<String>> initResourceAdapter(
        ...
        if (ijContainer.getResourceAdapterFactory().getClass().isAnnotationPresent(RunOnVirtualThread.class)) {
            WorkManager workManager = WorkManagerCoordinator.getInstance()
                    .createWorkManager(key, Defaults.VIRTUAL_THREAD_WORK_MANAGER_NAME);
            bootstrapContext.setWorkManager(workManager);
            bootstrapContext.setWorkManagerName(Defaults.VIRTUAL_THREAD_WORK_MANAGER_NAME);
        }
